...
const resolveUserRoleVariant = role => {
  ...
  if (r === '유저') return { color: 'success',  icon: 'bx-user' }
  if (r === '매니저')     return { color: 'error',    icon: 'bx-desktop' }
  if (r === '슈퍼어드민') return { color: 'info',     icon: 'bx-pie-chart-alt' }
  if (r === '지점장')     return { color: 'warning',  icon: 'bx-edit' }
  if (r === '어드민')      return { color: 'primary',  icon: 'bx-crown' }
